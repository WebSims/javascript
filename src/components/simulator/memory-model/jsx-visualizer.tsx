...
export const JsxVisualizer = () => {
    ...
    const transformMemorySnapshot = useCallback(() => {
    ...
    useEffect(() => {
        const data = transformMemorySnapshot()
        if (data) {
            calculateLayout(data).then(positionedData => {
                setVisualizationData(positionedData)
            })
        }
    }, [currentStep, transformMemorySnapshot, calculateLayout])
