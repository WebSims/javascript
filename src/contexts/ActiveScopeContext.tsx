...
export const ActiveScopeProvider = ({ 
  ...
  // If user steps the simulation, snap back to the current executing frame
  // (this keeps call stack selection aligned with the actual execution)
  useEffect(() => {
    if (!autoFollowDeepest) return
    if (!currentStep) return
    if (totalFrames <= 0) return

    setUserHasNavigated(false)
    setActiveFrameIndex(totalFrames - 1)
  }, [currentStep?.index, totalFrames, autoFollowDeepest])
